...
require_once 'connect.php';
	...
	$sql = "SELECT info,czas FROM ogloszenia ; " ;
	$result = $conn->query($sql);
	
	
		if ($result->num_rows > 0) {
    		echo '<table class="table table-hover"><tr><th>Info</th><th>Data dodania</th></tr>';
		    // output data of each row
		    while($row = $result->fetch_assoc()) {
		       echo '<tr><th scope="row">' . $row["info"]. "</th><td>" . $row["czas"]. "</td></tr>";
